...
sk.createSankey = function(containerId, configSankey, dataSankey) {
    ...
    var linearGradientGreen = defs.append( "linearGradient" )
      .attr( "id", "3db1a7" ) //unique id to reference the gradient by
      .attr( "x1", "0%" )
      .attr( "y1", "0%" )
      .attr( "x2", "100%" )
      .attr( "y2", "0" )
    ...
    var gradientStopsGreen = [ "rgba(0, 184, 159,1)", "rgba(0, 184, 159,0.5)", "rgba(0, 184, 159,1)" ];

    //Append the colors evenly along the gradient
    linearGradientGreen.selectAll( ".stop" )
      .data( gradientStopsGreen )
      .enter().append( "stop" )
      .attr( "offset", function ( d, i ) {
        return i / ( gradientStopsGreen.length - 1 );
      } )
      .attr( "stop-color", function ( d ) {
        return d;
      } );

    linearGradientGreen.append( "animate" )
    ...
    var linearGradientRed = defs.append( "linearGradient" )
      .attr( "id", "e65472" ) //unique id to reference the gradient by
      .attr( "x1", "0%" )
      .attr( "y1", "0%" )
      .attr( "x2", "100%" )
      .attr( "y2", "0" )
    ...
    var gradientStopsRed = [ "rgba(230, 84, 114,1)", "rgba(230, 84, 114,0.5)", "rgba(230, 84, 114,1)" ];

    //Append the colors evenly along the gradient
    linearGradientRed.selectAll( ".stop" )
      .data( gradientStopsRed )
      .enter().append( "stop" )
      .attr( "offset", function ( d, i ) {
        return i / ( gradientStopsRed.length - 1 );
      } )
      .attr( "stop-color", function ( d ) {
        return d;
      } );

    linearGradientRed.append( "animate" )
      .attr( "attributeName", "x1" )
      .attr( "values", "0%;100%" )
      .attr( "dur", "2s" )
      .attr( "repeatCount", "indefinite" );
    ...
    var link = svg.append("g").selectAll(".sk-link")
        ...
        .attr("class", "sk-link")
        .attr("d", path)
        // .style("stroke", "url(#linear-gradient-green)")
        // .style("stroke", function(l) {
        //     return l.source.color;
        // })
        .style("stroke", function(l) {
            return "url(" + l.source.color + ")"
        })
